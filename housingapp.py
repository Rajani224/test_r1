...
st.title('DreamHomes.com')                  # to run in anaconda 'streamlit run housingapp.py'
...
set_price=st.slider("Price RANGE",min_value=int(df['price'].min()), max_value=int(df['price'].max()), step=100, value=int(df['price'].min()))
...
st.set_option('deprecation.showPyplotGlobalUse',False) # to suppress the warning of old package
